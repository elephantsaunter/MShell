          ...
          <a-menu-item
            ...
            :class="{
              ...
              prohibit:
                ...
                store.state.itemRight.isAppItem,
            }"
            >刷新</a-menu-item
          >
...
const initRootDir = async () => {
  treeList.value[0].children = store.state.initData.ServerApp.AppList;
  treeList.value[0].isOpened = true;
  treeList.value[0].isSystem = true;
  treeList.value[0].children.forEach((item) => {
    item.basename = item.Name;
    item.children = item.DirList;
    item.isAppItem = true;
    item.isOpened = true;
    item.isSystem = true;
    (item.isMenu = true), (item.type = "directory");
...
//刷新
const refresh = (data) => {
  let item = data.filename ? data : store.state.itemRight;
  item.isLoad = true;
  if (item.basename == "应用列表") {
    //获取服务器基本信息
    getServerAppInfo({ pid: route.params.pid }).then((res) => {
      store.commit("SET_INIT_DATA", res);
      document.title = store.state.initData.Name;
      initRootDir();
      item.isLoad = false;
    });
    return;
...
const showModal = async () => {
  ...
  if (item.isAppItem) {
    formState.user.id = item.basename;
    select(item.Id);
  } else {
    appList.value.unshift({ Name: "新增应用", Id: 0 });
    formState.user.main_path = currentDirectory() == '/tree' ? '' : currentDirectory();
  }
  visible.value = true;
};
//关闭回调 重置表单
...
const select = (id = formState.user.id) => {
  if (formState.user.id === 0) {
    cancel();
    formState.user.main_path = currentDirectory() == '/tree' ? '' : currentDirectory();
  } else {
    appInfo({ pid: route.params.pid, id: id }).then((res) => {
      formState.user = res;
      console.log("96393", res);
